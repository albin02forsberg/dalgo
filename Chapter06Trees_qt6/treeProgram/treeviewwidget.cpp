...
auto SLUMPTALSMOTOR = std::default_random_engine {};
...
Node *TreeViewWidget::subtreeWorstcaseAVL(int height){
    ...
    pSub->m_pLeft = subtreeWorstcaseAVL(height-2);
    pSub->m_pRight = subtreeWorstcaseAVL(height-1);
    return pSub;
}
...
Node *TreeViewWidget::subtreeRandomAVL(int height, string path, const string &failpath){
    ...
    if (rand() % 2)
        std::swap( h1, h2 );
    Node *pSub = new Node(0);
    pSub->m_pLeft = subtreeRandomAVL(h1,  path+"0", failpath);
    pSub->m_pRight = subtreeRandomAVL(h2, path+"1", failpath);
    return pSub;
}
...
Node *TreeViewWidget::subtreeFromRandom(int depth)
{
    ...
    if (f< probabilityToCreate[depth]){
        pSub = new Node(0);
        pSub->m_pLeft =  subtreeFromRandom(depth+1);
        pSub->m_pRight = subtreeFromRandom(depth+1);
    }
    return pSub;
}
...
int TreeViewWidget::size(Node* pRoot){
    if (pRoot==nullptr)
        return 0;
    return size(pRoot->m_pLeft) + 1 + size(pRoot->m_pRight);
}
...
int TreeViewWidget::hOfTree(Node* pRoot){
    if (pRoot==nullptr)
        return -1;
    return 1+std::max(hOfTree(pRoot->m_pLeft), hOfTree(pRoot->m_pRight));
}
...
void TreeViewWidget::addParentlocationToNewnodes(Node *pRoot, QPointF locationOfParent, std::map<Node*,QPointF> &locations){
    ...
    addParentlocationToNewnodes(pRoot->m_pLeft,  locations[pRoot], (locations));
    addParentlocationToNewnodes(pRoot->m_pRight, locations[pRoot], (locations));
}
...
void TreeViewWidget::setDesiredLocations(Node *pRoot, int depth, int& x, float dx, float dy, std::map<Node*,QPointF> &locationFromNode){
    if (pRoot==nullptr)
        return;
    setDesiredLocations(pRoot->m_pLeft, depth+1, (x), dx,dy,  (locationFromNode) );
    locationFromNode[pRoot] = QPointF(x*dx, depth*dy);
    x += 1;
    setDesiredLocations(pRoot->m_pRight, depth+1, (x), dx,dy, (locationFromNode));
}
...
int TreeViewWidget::nextkeyWhenDoAssignKeyvalues(Node *pSubtree, int firstValue){
    ...
    int keyRoot = nextkeyWhenDoAssignKeyvalues(pSubtree->m_pLeft, firstValue + 2+rand()%5);
    pSubtree->m_key = keyRoot;
    int keyNext = nextkeyWhenDoAssignKeyvalues(pSubtree->m_pRight, keyRoot+2+rand()%5);
    return keyNext;
}
...
{
    if (pRoot!=nullptr){
        destroySubtree(pRoot->m_pLeft);
        destroySubtree(pRoot->m_pRight);
        delete pRoot;
    }
}
...
void TreeViewWidget::createTestTree1()
{
...
bool TreeViewWidget::isAVL(Node *pTree, int *pHeight){
    ...
    int heightLeft = 0;
    int heightRight = 0;
    bool ok = isAVL(pTree->m_pLeft, (&heightLeft)) && isAVL(pTree->m_pRight, (&heightRight));
...
void TreeViewWidget::createRandomSearchTree(int size)
{
    ...
    std::shuffle(keys.begin(), keys.end() , SLUMPTALSMOTOR);
...
int TreeViewWidget::sumOfDepth(Node *pRoot, int depthOfThisNode){
    ...
    return depthOfThisNode
            + sumOfDepth(pRoot->m_pLeft, depthOfThisNode+1)
            + sumOfDepth(pRoot->m_pRight, depthOfThisNode+1);
}
int TreeViewWidget::antalNoder(Node *pRoot){
    if (pRoot==nullptr)
        return 0;
    return 1 + antalNoder(pRoot->m_pLeft)+ antalNoder(pRoot->m_pRight);
}
...
void TreeViewWidget::paintEvent(QPaintEvent *event)
{
    ...
    if (_pNodeUnderMouse){
        ...
        QString str = "key = " + QString::number(_pNodeUnderMouse->m_key) + "    ";
...
bool TreeViewWidget::readStream(ifstream& infil)
{
    destroySubtree(_pRoot);
    _pRoot = ::readTreeFromStream(infil);
    _locationFromNodeptr.clear();
    animateTheChange();
    update();
    return true;
...
bool TreeViewWidget::saveStream(ofstream& utfil)
{
    ::saveTreeToStream(utfil, _pRoot);
    update();
    return true;
}
...
bool TreeViewWidget::containedInSerachTree(int key)
{
    bool found = ::contains(_pRoot, key);
    return found;
}

void TreeViewWidget::removeKeyFromSearchtree(int key)
{
    assert( std::string(nameOfStudent2()) != "Homer Simpson" );
...
void TreeViewWidget::showContextMenu(const QPoint &point)
{
    ...
    if (selected!=nullptr){
        if (selected==actionDelete)
            ...
            assert( std::string(nameOfStudent4()) != "Homer Simpson" );

            ::hasPathToTarget(_pRoot);
...
void TreeViewWidget::drawArcs(QPainter &painter, QPointF locationOfParent, Node* pSubtree ){
    ...
    drawArcs(painter, location, pSubtree->m_pLeft);
    drawArcs(painter, location, pSubtree->m_pRight);
}
...
void TreeViewWidget::fillNodesInSubtree(QPainter &painter, Node* pNode){
    ...
    fillNodesInSubtree(painter, pNode->m_pLeft);
    fillNodesInSubtree(painter, pNode->m_pRight);
}
...
void TreeViewWidget::fillNode(QPainter &painter, Node* pNode){
    ...
    else painter.setBrush(colorFromKey(pNode->m_key));
    ...
    painter.setFont(font);
    if (_shouldShowKey)
        painter.drawText(rc, Qt::AlignCenter, QString::number(pNode->m_key));
...
void TreeViewWidget::testfunction1(){
    ::testfunktion1(_pRoot);
}

void TreeViewWidget::testfunction2(){
    ::testfunktion2(_pRoot);

}

void TreeViewWidget::testfunction3(){
    ::testfunktion3(_pRoot);

}

void TreeViewWidget::testfunction4(){
    ::testfunktion4(_pRoot);
